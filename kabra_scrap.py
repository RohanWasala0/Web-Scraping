...
temp = list(x.text for x in pair_numbers if x.text != '')
print(temp)
...
if input_pair_number in temp:
    ...
    table_head_elements = table_element.find_elements(By.XPATH, './/thead/tr')
    table_body = table_element.find_element(By.TAG_NAME, 'tbody')

    fTable_head = []
    for m in table_head_elements:
        table_head_data = list(x.text for x in m.find_elements(By.TAG_NAME, 'th') if x.text != '')
        fTable_head += table_head_data
    print(fTable_head)
    ...
    df.to_csv(f"{input_pair_number}_kabra.csv", index=False)
else:
